...
**Fan Control Pin**: GPIO0 (moved from GPIO21 to free SPI SCK)
...
void vSystemMonitorTask(void *pvParameters) {
    ...
    for (;;) {
        ...
        if (temp_valid) {
            ...
            gpio_set_level(GPIO_NUM_0, 1);  // Fan ON (safe default)
        ...
        if (log_counter >= 5) {
            ...
            bool fan_state = gpio_get_level(GPIO_NUM_0);
...
- **Fan State**: "ON" or "OFF" (based on GPIO0 level)
...
cJSON_AddBoolToObject(json, "fan_on", gpio_get_level(GPIO_NUM_0));
...
#define FAN_CONTROL_GPIO_PIN                  GPIO_NUM_0
...
#define PIN_FAN_CONTROL    GPIO_NUM_0  // Fan control output pin
```
